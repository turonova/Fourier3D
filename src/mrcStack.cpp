...
using namespace std;
	...
	MRCStack::MRCStack(string aFileName, bool getRange, bool swapYZ, bool keepOpen)
	{
		...
		if(swapYZ)
		{
			fourier::swap<int>(mHeaderMRC.ny,mHeaderMRC.nz);
			fourier::swap<int>(mHeaderMRC.my,mHeaderMRC.mz);
			fourier::swap<int>(mHeaderMRC.nyStart,mHeaderMRC.nzStart);
			fourier::swap<float>(mHeaderMRC.cellDimY,mHeaderMRC.cellDimZ);
			fourier::swap<float>(mHeaderMRC.cellAngleY,mHeaderMRC.cellAngleZ);
			fourier::swap<int>(mHeaderMRC.mapR,mHeaderMRC.mapS);
		}
	...
	fourier::Vec3ui MRCStack::getResolution()
	{
		return fourier::Vec3ui(mHeaderMRC.nx, mHeaderMRC.ny, mHeaderMRC.nz);
	}
